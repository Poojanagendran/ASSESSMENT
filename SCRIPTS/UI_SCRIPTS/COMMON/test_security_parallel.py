...
class TestSecurity:
    ...
    def test_security_case(self, candidate_details):
        self.row = self.row + 1
    ...
    def run_test_in_parallel(self, excel_data):
        processes = []
        for current_excel_row in excel_data:
            print(f"Running test case: {current_excel_row.get('testCases')}")
            process = Process(target=self.test_security_case, args=(current_excel_row,))
...
test_security_obj.run_test_in_parallel(excel_data)
