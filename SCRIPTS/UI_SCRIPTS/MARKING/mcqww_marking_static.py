...
class OnlineAssessment:
    ...
    def __init__(self):
        self.row = 1
        write_excel_object.save_result(output_path_ui_mcqww_marking_schema)
        header = ['mcqww test marking']
    ...
    def mcqww_marking(self, tu_details, token):
        ...
        group2_section1_obtained_mark = mark_details['data']['groupAndSectionWiseMarks'][1]['sectionInfo'][0][
            'obtainedMarks']
        all_questions = mark_details['data']['mcqWithWeightage']
        # id = 121404
...
excel_read_obj.excel_read(input_path_ui_marking_schema, 2)
...
for current_excel_row in excel_data:
    ...
    assessment_obj.mcqww_marking(current_excel_row, crpo_token)
