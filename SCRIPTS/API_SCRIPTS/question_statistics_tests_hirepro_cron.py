...
from SCRIPTS.DB_DELETE.question_statistics_test_new_cron_delete import qs_new_cron_test_del
...
class OnlineAssessment:
    ...
    def __init__(self):
        self.row = 1
        qs_new_cron_test_del.delete_question_statistics()
        write_excel_object.save_result(output_question_statistics_tests_hp_cron)
    ...
    def calculate_qn_stats(self, excel_datas, automation_token):
        ...
        if automation_test_ids:
            for test_id in automation_test_ids:
                ...
                calculate_question_stats_context_id = CrpoCommon.calculate_question_statistics_for_tests(
                    automation_token,
                    test_id)
                    ...
                    for act_question in question_stats1:
                        ...
                        for excel_question in excel_datas:
                            if act_question.get('questionId') == int(excel_question.get('questionId')):
                                ...
                                act_avg_response_time = round(act_question.get('avgResponseTime'), 4)
                                act_exposure_rate = act_question.get('exposureRate')
                                act_facility_value = round(act_question.get('facilityValue'), 4)
                                act_item_difficulty = round(act_question.get('itemDifficulty'), 4)
                                act_dist_index_of_a = round(act_question.get('distractorIndexOfA'), 4)
                                act_dist_index_of_b = round(act_question.get('distractorIndexOfB'), 4)
                                act_dist_index_of_c = round(act_question.get('distractorIndexOfC'), 4)
                                act_dist_index_of_d = round(act_question.get('distractorIndexOfD'), 4)
                                act_no_of_attempted_a = round(act_question.get('numberOfAttendedA'), 4)
                                act_no_of_attempted_b = round(act_question.get('numberOfAttendedB'), 4)
                                act_no_of_attempted_c = round(act_question.get('numberOfAttendedC'), 4)
                                act_no_of_attempted_d = round(act_question.get('numberOfAttendedD'), 4)
                                self.row = self.row + 1
                                ...
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('avgResponseTime'), 4), act_avg_response_time, self.row, 7)
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('itemDifficultyLevel'), 4), act_item_difficulty, self.row,
                                    9)
                                write_excel_object.compare_results_and_write_vertically(
                                ...
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('facilityValue'),4),
                                ...
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('distractorIndexA'),4), act_dist_index_of_a, self.row, 21)
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('distractorIndexB'),4), act_dist_index_of_b, self.row, 23)
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('distractorIndexC'),4), act_dist_index_of_c, self.row, 25)
                                write_excel_object.compare_results_and_write_vertically(
                                    round(excel_question.get('distracotrIndexD'),4), act_dist_index_of_d, self.row, 27)
...
excel_read_obj.excel_read(input_question_statistics_tests, 1)
